...
class GameCell: UITableViewCell {
    ...
    // 이미지 캐시
    private static var imageCache = NSCache<NSString, UIImage>()import UIKit
...
class GameCell: UITableViewCell {
    ...
    // MARK: - 팀 ID → ESPN 약자 매핑
    private func espnAbbr(teamId: Int) -> String? {
        let map: [Int: String] = [
    ...
    private func loadLogo(teamId: Int, into imageView: UIImageView) {
        guard let abbr = espnAbbr(teamId: teamId) else {
            imageView.image = UIImage(systemName: "baseball.fill")
            imageView.tintColor = .systemGray3
            return
        }

        let urlString = "https://a.espncdn.com/i/teamlogos/mlb/500/\(abbr).png"
        ...
        URLSession.shared.dataTask(with: url) { data, response, error in
            if let error = error {
                print("❌ 이미지 오류 (teamId: \(teamId)):", error.localizedDescription)
                return
            }

            if let httpResponse = response as? HTTPURLResponse {
                print("📡 상태코드 (teamId: \(teamId)):", httpResponse.statusCode)
                guard httpResponse.statusCode == 200 else { return }
            }

            guard let data = data,
                  let image = UIImage(data: data) else {
                print("❌ 이미지 변환 실패 (teamId: \(teamId))")
                return
            }

            print("✅ 이미지 성공 (teamId: \(teamId))")
